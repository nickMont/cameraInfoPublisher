...
public:
  PublishCameraInfo()
  {
    ...
    imagepub_ = n_.advertise<sensor_msgs::Image>("/published_topic", 1);

    //Topic you want to subscribe
    sub_ = n_.subscribe("/subscribed_topic", 1, &PublishCameraInfo::callback, this);
  }

  void callback(const sensor_msgs::CompressedImage jpegmsg)
  {

    sensor_msgs::CameraInfo cameraPub;
    sensor_msgs::Image imagePub;
    ...
    imagePub.encoding="RGB8";
    imagePub.step=2160;

    imagePub.header.frame_id=jpegmsg.header.frame_id;
    imagePub.header.stamp=jpegmsg.header.stamp;
    cameraPub.header.frame_id=jpegmsg.header.frame_id;
    cameraPub.header.stamp=jpegmsg.header.stamp;
    imagePub.data=jpegmsg.data;  //get raw image data from image_raw ros stream
    imagepub_.publish(imagePub);
...
};//End of class PublishCameraInfo
